...
Notebook version: returns a Bokeh Figure object directly so it can be
rendered inline with show() inside a widgets.Output context.
"""
import math
...
def build_enrollment_figure(
    ...
    """
    Returns a Bokeh Figure with pessimistic / moderate / optimistic curves.
    Solid lines = cumulative patients (left axis).
    Dashed lines = active sites (right axis).
    """
    colors = {
        "pessimistic": "#e74c3c",
        "moderate": "#2980b9",
    ...
    for scenario_name, params in scenarios.items():
        ...
        l_patients = p.line(
            result["dates"], result["cumulative_patients"],
            line_color=color, line_width=2.5, line_dash="solid",
        )
        l_sites = p.line(
            result["dates"], result["active_sites"],
            line_color=color, line_width=1.5, line_dash="dashed",
            y_range_name="sites",
        )
        legend_items.append((f"{scenario_name.capitalize()} — Patients (solid)", [l_patients]))
        legend_items.append((f"{scenario_name.capitalize()} — Sites (dashed)", [l_sites]))

    all_dates_end = start_date + timedelta(days=30 * 72)
    p.line(
        [start_date, all_dates_end],
        [num_patients, num_patients],
        line_dash="dotted", line_color="gray", line_width=1.5,
        legend_label=f"Target ({num_patients} patients)"
    )

    hover = HoverTool(tooltips=[("Date", "@x{%b %Y}")], formatters={"@x": "datetime"})
    p.add_tools(hover)
